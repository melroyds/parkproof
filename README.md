...
- **Sign in with Apple.** Frontend is wired (button + callback handler) and the hosted-UI infrastructure is provisioned — only the Apple side is pending: Services ID + Team ID + Key ID + `.p8` private key, then the two `aws cognito-idp` registration commands from [`docs/federation-setup.md`](docs/federation-setup.md). Apple Developer subscription is paid; waiting on account provisioning. Google federation is already live.
